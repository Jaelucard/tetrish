...
static htttp_err_t parse_body(htttp_msg_t *msg, const char *cur, const char *end) {
    if (msg->content_length <= 0) {
        if (cur < end) {
            /* trailing bytes with no (or zero) Content-Length: malformed,
             * same reasoning as the request parser. */
...
htttp_err_t htttp_parse_request(const char *buf, size_t buf_len, htttp_msg_t *msg) {
    ...
    if (*(sp1 + 1) != '/') {
        return HTTTP_ERR_MALFORMED;   /* edge case: path must be absolute ("no leading slash") */
    }
    ...
    if (msg->content_length > 0) {
        size_t remaining = (size_t)(end - cur);
        if (remaining < (size_t)msg->content_length) {
            return HTTTP_ERR_INCOMPLETE;     /* Content-Length says more than we got */
        }
        if (remaining > (size_t)msg->content_length) {
            return HTTTP_ERR_MALFORMED;      /* Content-Length says less than we got */
        }
        msg->body = cur;
        msg->body_len = (size_t)msg->content_length;
    } else if (cur < end) {
        /* Edge case: bytes follow the blank line but there was no (or a
         * zero) Content-Length header. HTTTP/1.0 has no chunked encoding
         * and no other way to frame a body, so this is unambiguously
         * malformed rather than data we can silently drop. */
        return HTTTP_ERR_BAD_HEADER;
    }
