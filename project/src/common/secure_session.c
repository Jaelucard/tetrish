/*
 * Standardised secure-session library shared by tetriSH (tetrisd/tetrisu) and mini-gh-tracker.
 *
 ...
 *
 * Wire format for every message after the handshake: [ 4-byte big-endian length ][ IV (16 bytes) ][ AES-256-CBC ciphertext ]
 * The length field counts only the IV + ciphertext, not itself. 
 * Frames larger than SESSION_MAX_FRAME_LEN (64 KiB payload) are rejected.
 */
...
static int recv_all(int fd, void *buf, size_t len) {
    ...
    while (len > 0) {
        ...
        if (n <= 0) {
            return -1;   /* 0 = peer closed cleanly */
        }
        p += n; len -= (size_t)n;
    }
    return 0;
...
static int aes256_cbc_encrypt(const unsigned char *key, const unsigned char *plain, size_t plain_len, unsigned char **out, size_t *out_len) {
...
session_t *session_accept(session_ctx_t *ctx, int raw_fd) {
    ...
    unsigned char *enc_key = recv_length_prefixed(raw_fd, &enc_key_len, 4096);
...
session_t *session_connect(session_ctx_t *ctx, int raw_fd) {
    ...
    unsigned char *cert_pem = recv_length_prefixed(raw_fd, &cert_pem_len, 64 * 1024);
    ...
    size_t sig_len = 0;
    unsigned char *sig = recv_length_prefixed(raw_fd, &sig_len, 4096);
...
static int session_send_frame(session_t *sess, const unsigned char *plaintext, size_t plain_len) {
    ...
    free(frame);
    if (rc != 0) { 
        _err(sess, "session_write: send failed"); 
        return -1;
    }
    return 0;
}
...
static ssize_t session_recv_frame(session_t *sess) {
    size_t frame_len = 0;
    unsigned char *frame = recv_length_prefixed(sess->sockfd, &frame_len, SESSION_MAX_FRAME_LEN + 16 + 16);
    if (!frame) { 
...
ssize_t session_read(session_t *s, void *buf, size_t len) {
    if (!s || !session_is_ready(s) || !buf || len == 0) {
        if (s) {
            _err(s, "session_read: invalid arguments or session not ready");
        }
        return -1;
    }
    ...
    if (s->rbuf_off >= s->rbuf_len) {
        ...
        if (rc == 0) {
            return 0; // empty frame: treat like EOF-of-frame, caller retries
        }
    }
...
ssize_t session_write(session_t *s, const void *buf, size_t len) {
    if (!s || !session_is_ready(s) || !buf) {
        if (s) {
            _err(s, "session_write: invalid arguments or session not ready");
        }
        return -1;
    }
    ...
    if (session_send_frame(s, buf, len) != 0) {
        return -1;
    }
