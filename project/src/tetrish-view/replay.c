// Implementation of replay.h. See replay.h and main.c's usage comment for
// the CLI-facing contract; this file owns the actual reconstruction.
//
...
// SIGINT handler: flag only, so the viewer loop can end the frame cleanly.
...
// maps a recorded INPUT token back to the engine input it logged. an
// unknown name becomes NONE, so an unrecognised record cannot corrupt the
// reconstruction -- it just does nothing that tick.
...
// draws the board's box-drawing border for the interactive viewer.
...
// draws one frame of the interactive viewer: the reconstructed board plus
// a status block that always states how trustworthy the reconstruction is
// (verified tick, mismatch count, truncation).
...
// the whole replay feature behind one call: load the log, rebuild the
// session by re-driving tb_tick, then either print a headless verification
// report (verify != 0) or run the interactive ncurses viewer. both the CLI
// and the repl's `replay` command land here. contract in replay.h.
int replay_run(const char *logpath, const char *want_room,
               const char *want_player, int verify){
    // g_ev/g_nev/g_quit are process-wide statics (see above): a second call
    // in the same process (e.g. the repl running `replay` twice in one
    // session) must not accumulate events from the previous call.
    g_nev = 0;
    g_quit = 0;
    ...
    if (rc == -2 || g_nev == 0){
        fprintf(stderr,
                ...
                "  try naming a room and player, e.g. replay %s roomA p7\n",
                logpath, logpath);
        return 1;
    }
    ...
    // The interactive viewer needs a real terminal. Without this check,
    // initscr() against a pipe/redirect either fails obscurely or wedges the
    // caller -- a real risk now that `replay` is also reachable as a repl
    // command that could be driven from a script (tests/integration.sh).
    if (!isatty(STDOUT_FILENO)){
        fprintf(stderr,
                "tetrish-view: interactive replay needs a terminal "
                "(use --verify from the command line instead)\n");
        return 2;
    }
